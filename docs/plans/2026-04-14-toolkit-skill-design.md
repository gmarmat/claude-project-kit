...
**Reference implementations** (patterns observed across prior projects, generalized for this kit):
- A 7-tab PM Intelligence Hub with sales calculator, market data, pricing models, competitive analysis, technical positioning, demo mode. Hardcoded HTML/JS with cited sources.
- A product-infrastructure layer: monitoring dashboard (4 tabs), A/B testing framework (5 DB tables), admin dashboards (4 pages), AI insights (6 views), settings system (8 tabs + audit log).
...
**Key patterns to preserve from the reference implementation:**
...
- Generic domain — no industry-specific terminology
...
| 5 | Should the calculator support custom formulas or just the tier-based reference pattern? | Flexibility | **Recommend: tier-based pricing as default, extensible** |
